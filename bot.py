...
async def main_async():
    ...
    try:
        ...
        # Запускаем polling с drop_pending_updates=True
...
def main():
    """Точка входа - исправленная версия для работы с существующим event loop"""
    try:
        ...
        if loop.is_running():
            # Loop уже запущен (например, на хостинге)
            ...
            logger.info("🔄 Event loop exists but not running, using run_until_complete")
            loop.run_until_complete(main_async())
            
    except RuntimeError:
        # Нет активного event loop
        logger.info("🔄 No running event loop, using asyncio.run()")
...
# Альтернативный вариант для хостингов с уже запущенным loop
async def start_bot():
    """Асинхронная точка входа для хостингов, поддерживающих async"""
    try:
        await delete_webhook()
        await asyncio.sleep(1)
        init_database()
        ...
        logger.info("🚀 Bot started successfully (async entry point)")
        await app.run_polling(allowed_updates=Update.ALL_TYPES, drop_pending_updates=True)
...
if __name__ == '__main__':
    ...
    # Проверяем, запущены ли мы в среде с уже работающим event loop
    try:
        loop = asyncio.get_running_loop()
        # Если мы здесь, значит loop уже запущен (например, на хостинге)
        logger.info("🌐 Running in environment with active event loop")
        loop.create_task(start_bot())
        # Держим loop активным
        loop.run_forever()
    except RuntimeError:
        # Нет активного loop - используем стандартный запуск
