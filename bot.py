...
# Функция для удаления вебхука и предотвращения конфликтов
async def delete_webhook():
    """Принудительное удаление webhook перед запуском"""
    try:
        bot = Bot(token=BOT_TOKEN)
        await bot.delete_webhook(drop_pending_updates=True)
        print("✅ Webhook удален, конфликтов не будет")
...
async def main_async():
    ...
    # Удаляем webhook перед запуском для предотвращения конфликтов
    ...
    # Callback handlers - все кнопки из админки и подменю
...
def main():
    """Синхронная функция main для запуска"""
